...
function parse_contents($contents) // Returns true if invalid person.  Invalid action is checked elsewhere
{
	...
	else
	{
		...
		if (!in_array($person, array(strtolower(FTGR_REEMON_NAME), strtolower(FTGR_PLUFF_NAME), strtolower(FTGR_DRAGIRI_NAME), strtolower(FTGR_NIGHTWING_NAME)))) // Prevent access of unwanted variables
		{
			return true;
		}
	}
