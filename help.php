...
<BODY>
	...
	<p>If you are going to test a non-stable build, you need to set up a PHP server to play on.  I highly recommend <a href="http://www.apachefriends.org/en/xampp.html">XAMPP</a>.  Download the FTG:R files to (XAMPP install directory)/xamppfiles/htdocs/ftgr.  Open the XAMPP manager and start the Apache module (you can start mySQL and proFTPD, but they are not required).  Then go to http://localhost/ftgr to play (you can rename the folder too, I personally name mine ftgr-dev).  Some applications, like Skype, Internet Information Services, and many game development softwares use localhost, so you will need to quit these programs completely. </p>
	<p>Although this application is written in PHP, it is <em>not</em> recommended that you put any dev, alpha, beta, or release candidate on a production server.  There may be server-breaking bugs, and the source code is not tested thoroughly before release to GitHub.  In addition, the documentation is likely to be out of date and functions may change dramatically between releases.  The debug function allows access to all $GLOBALS, and the site is only configured to work on a localhost server.</p>
	<p>As of version 0.3, Javascript must be enabled in the browser for any function to work.  <script>document.write('You have Javascript enabled, so there is no need to worry.');</script><noscript>You do not have javascript enabled (or Javascript has not yet loaded), so please enable it to use FTG:R.</noscript></p>
	<h3>Windows</h3>
